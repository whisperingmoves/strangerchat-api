...
module.exports = async (io, userIdSocketMap, userId, data) => {
    try {
        ...
        // 查找对方用户信息
        const opponentUserId = conversation.userId1.toString() === userId ? conversation.userId2 : conversation.userId1;
        ...
        const chatConversationDetails = {
            ...
            opponentDistance,
            lastMessageTime: conversation.lastMessageTime ? Math.floor(conversation.lastMessageTime.getTime() / 1000) : undefined,
        ...
        if (socketIds && socketIds.length > 0) {
            socketIds.forEach(socketId => {
                io.to(socketId).emit('notifications', jsonData);
            });
        }
    } catch (error) {
        console.error('Error in getChatConversationDetails socket controller:', error);
    }
};
