...
module.exports = async (io, userIdSocketMap, userId, data) => {
    try {
        ...
        if (currentUser.location && currentUser.location.coordinates && opponentUser.location && opponentUser.location.coordinates) {
        ...
        if (currentUserSockets) {
            currentUserSockets.forEach(socketId => {
                io.to(socketId).emit('notifications', { type: 3, data: currentUserData });
            });
        }
        ...
        if (opponentUserSockets) {
            opponentUserSockets.forEach(socketId => {
                io.to(socketId).emit('notifications', { type: 3, data: opponentUserData });
            });
        }
    } catch (error) {
