...
const getStoryList = async (req, res, next) => {
    ...
    try {
        ...
        const formattedPosts = await Promise.all(posts.map(async (post) => {
            const { _id, avatar, username, createdAt: userCreatedAt, online } = post.author;
            const { createdAt, images } = post;
            const userId = _id.toHexString();
            const currentUser = req.user ? await User.findById(req.user.userId) : undefined;
            ...
            if (!currentUser || currentUser._id.equals(_id)) {
                ...
                const diffDays = Math.round((nowTimestamp - userCreatedAtTimestamp) / (1000 * 60 * 60 * 24));
                ...
                if (diffDays <= 7) {
                    ...
                    if (isFollowing && author.following.includes(currentUser._id)) {
                        relation = 0;
                    } else if (!isFollowing && author.following.includes(currentUser._id)) {
                        relation = 1;
                    } else if (isFollowing && !author.following.includes(currentUser._id)) {
                        relation = 2;
                    }
